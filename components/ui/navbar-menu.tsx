          ...
          {active === item && (
            <div className="fixed md:absolute top-[70px] md:top-[calc(100%_+_1.2rem)] left-0 md:left-1/2 right-0 md:right-auto md:-translate-x-1/2 pt-4 z-[100] px-4 md:px-0">
              <motion.div
                transition={transition}
                layoutId="active"
                className="bg-[#0a0a0a]/95 backdrop-blur-2xl rounded-2xl overflow-hidden border border-white/10 shadow-2xl max-h-[60vh] md:max-h-[70vh] overflow-y-auto"
              >
                <motion.div
                  layout
  ...
  return (
    <a href={href} className="flex flex-col md:flex-row space-y-2 md:space-y-0 md:space-x-3 group">
      ...
      <div className="flex-1">
        <h4 className="text-base md:text-lg font-bold mb-1 text-white line-clamp-1">
          {title}
        </h4>
...
export const HoveredLink = ({ children, ...rest }: any) => {
  return (
    <a
      {...rest}
      className="block text-gray-400 hover:text-[#4A90E2] transition-colors"
    >
      {children}
    </a>
  );
