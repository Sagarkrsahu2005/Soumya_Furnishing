  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        ...
        <button onClick={resetFilters} className="text-sm text-[#4A90E2] hover:text-[#7CB342] transition-colors">
          Reset
        </button>
      </div>
      ...
      <div className="space-y-4">
        <button
          ...
          className="flex items-center justify-between w-full pb-3 border-b border-white/10"
        >
          <h4 className="font-semibold text-white">Room</h4>
          <ChevronDown
            className={`w-4 h-4 text-gray-400 transition-transform ${expandedSections.includes("room") ? "rotate-180" : ""}`}
          />
        </button>
        {expandedSections.includes("room") && (
          <div className="space-y-2">
            {rooms.map((room) => (
              <label key={room} className="flex items-center gap-2 cursor-pointer hover:bg-white/5 p-1 rounded transition-colors">
                <input
                  type="checkbox"
      ...
      <div className="space-y-4">
        ...
        >
          ...
          <ChevronDown
            className={`w-4 h-4 text-gray-400 transition-transform ${expandedSections.includes("material") ? "rotate-180" : ""}`}
          />
        </button>
        {expandedSections.includes("material") && (
          <div className="space-y-2">
            {materials.map((material) => (
              <label key={material} className="flex items-center gap-2 cursor-pointer hover:bg-white/5 p-1 rounded transition-colors">
                <input
                  type="checkbox"
            ...
            {colors.map((color) => (
              <button
                ...
                className={`aspect-square rounded-full border-2 transition-all ${
                  filters.colors.includes(color) ? "border-[#4A90E2] scale-110" : "border-white/20"
                }`}
                title={color}
              />
            ))}
      ...
      <div className="space-y-4">
        <button
          ...
          className="flex items-center justify-between w-full pb-3 border-b border-white/10"
        >
          <h4 className="font-semibold text-white">Collections ({allCollections.length})</h4>
          <ChevronDown
            className={`w-4 h-4 text-gray-400 transition-transform ${expandedSections.includes("collections") ? "rotate-180" : ""}`}
          />
        </button>
          ...
          <div className="space-y-3">
            <div className="relative">
              <input
                ...
                className="w-full px-3 py-2 pl-8 text-sm border border-white/20 bg-[#2d2d2d] text-white rounded focus:outline-none focus:border-[#4A90E2] transition-colors"
              />
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {allCollections
                ...
                .map((collection) => (
                  <label key={collection.id} className="flex items-center gap-2 cursor-pointer hover:bg-white/5 p-1 rounded">
                    <input
                      type="checkbox"
                      ...
                      className="w-4 h-4 accent-[#4A90E2]"
                    />
                    <span className="text-sm text-gray-300">{collection.title}</span>
                  </label>
                ))}
            </div>
          </div>
